...
impl cosmic::Application for AppModel {
            ...
            Message::OpenTab(name) => {
                ...
                #[cfg(target_os = "linux")]
                {
                    ...
                    if let Some(window_id) = self.main_window_id {
                        let wv_task =
                            build_webview_task(entity, name, window_id, w, h);
                        return Task::batch([title_task, wv_task]);
                    }
                    // Window ID not yet known (shouldn't happen in practice since
                    // GotMainWindowId fires during init, but be safe).
                    tracing::warn!("OpenTab: main_window_id not yet set; webview skipped");
                }
                ...
                match self.geese_storage.create(trimmed) {
                    Ok(profile) => {
                        ...
                        if let Some(window_id) = self.main_window_id {
                            ...
                            let (w, h) = self.window_size;
                            let wv_task =
                                build_webview_task(entity, profile_name, window_id, w, h);
                            return Task::batch([title_task, wv_task]);
                        }
    ...
    fn subscription(&self) -> Subscription<Self::Message> {
        ...
        #[cfg(target_os = "linux")]
        let gtk_pump = cosmic::iced::time::every(Duration::from_millis(16))
            .map(|_| Message::PumpGtk);
