...
impl AppModel {
    ...
    ///
    /// libcosmic draws a system-style header (menu / gear / "+ New tab" /
    /// minimise / maximise / close) *above* the iced viewport. iced's
    /// `rectangle_tracker` reports the tab body's position in
    /// **viewport-local** coords (y=0 = top of iced's drawable area), but
    /// `wry::WebViewBuilder::build_as_child` positions the child X window in
    /// **parent-window** coords (y=0 = top of the X window, header included).
    /// On the COSMIC build this header is `webview::COSMIC_HEADER_HEIGHT`
    /// pixels, so we shift wry's rect down by that amount and shrink the
    /// height to match so the webview's bottom stays inside the tab body.
    ///
    /// `GANDER_WEBVIEW_X_OFFSET` / `GANDER_WEBVIEW_Y_OFFSET` env vars still
    /// override on top of the bake-in for debugging on other COSMIC builds.
    #[cfg(target_os = "linux")]
    fn webview_bounds(&self) -> (f64, f64, f64, f64) {
        ...
        let y_off: f64 = std::env::var("GANDER_WEBVIEW_Y_OFFSET")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(webview::COSMIC_HEADER_HEIGHT);
